...
`--created-at`, `--started-at`, and `--ended-at` take RFC 3339 timestamps (normalized to UTC, rejected before anything is written when invalid) and set the new meeting's metadata — `--created-at` backdates it on the timeline and in its folder name, which is what you want when importing a historical recording. `--author NAME` records who created the meeting; agents importing on a user's behalf should always set it, and `--skill NAME` additionally records the skill used, if any (see `meetings new`). Because they describe a new meeting, all of these are rejected together with `--into`.
