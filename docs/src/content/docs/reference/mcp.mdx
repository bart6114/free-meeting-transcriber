...
Each returned meeting includes its normalized `tags`.
...
Accepts required string `meeting_id`. Returns meeting metadata (including its normalized `tags`), the canonical note, summaries, and action items. Transcript words are intentionally separate.
