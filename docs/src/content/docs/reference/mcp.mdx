...
Each returned meeting includes its normalized `tags` and its `author` (`null` when the vault owner wrote the note; otherwise the name of the agent or other writer that created it).
...
Accepts required string `meeting_id`. Returns meeting metadata (including its normalized `tags` and its `author` — `null` for the vault owner's own notes), the canonical note, summaries, and action items. Transcript words are intentionally separate.
