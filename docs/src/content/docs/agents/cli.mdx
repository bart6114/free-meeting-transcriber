...
description: "Use fmtr to search the local knowledge vault and create notes from agent work or external sources."
---
...
```bash
fmtr --json sessions list --query "weekly planning" --limit 10
...
```bash
fmtr --json sessions note SESSION_ID --kind note
fmtr --json sessions note SESSION_ID --kind summary
```
...
```bash
fmtr --json sessions new --title "Weekly planning" --note notes.md --author claude-code
fmtr --json sessions new --title "Research: payment providers" --note research.md --tag research --author claude-code --skill research-assistant
fmtr --json sessions note SESSION_ID --append notes.md
fmtr --json sessions attach SESSION_ID source-deck.pdf
fmtr --json import recording.m4a --transcribe --author claude-code
fmtr --json transcribe SESSION_ID
```

`sessions new` creates a session and prints its ID; `--note` seeds the note body from a file or stdin (`-`). The session can stand alone as a note, with no recording or transcript. This lets an agent store a research brief, extracted deck insights, project context, or other useful output from any source it is authorized to access. The agent performs the source integration and supplies Markdown; `fmtr` provides the stable write path into the vault.
...
```bash
fmtr sessions export SESSION_ID --format markdown --output session.md
fmtr sessions export SESSION_ID --format json --output session.json
```
