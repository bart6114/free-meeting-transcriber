...
```bash
fmtr --json meetings new --title "Weekly planning" --note notes.md --author claude-code
...
**Always pass `--author <your-agent-name>` when creating a meeting** (`meetings new`, `import`) — it marks the note as not written by the vault owner, and the app surfaces that. Use one stable name, such as `claude-code`. Write your own notes as new meetings with `--author`; when editing an existing note, never change its authorship.
