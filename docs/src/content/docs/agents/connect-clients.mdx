---
title: "Connect Loofah to Claude, Codex, or Hermes"
description: "Use Loofah's read-only MCP server with Claude Desktop, Claude Code, Codex, or Hermes."
---

Loofah's MCP server lets an agent search and read your local meeting notes. It cannot create sessions, edit notes, or change the vault.

Install the [`loofah` CLI](/installation) first, then check that it can find your vault:

```bash
loofah doctor
```

Every client starts the same local command: `loofah mcp`.
...
```json
{
  "mcpServers": {
    "loofah": {
      "command": "/Users/you/.local/bin/loofah",
      "args": ["mcp"]
    }
  }
}
...
```bash
claude mcp add --transport stdio loofah -- loofah mcp
claude mcp list
```
...
```bash
codex mcp add loofah -- loofah mcp
codex mcp list
```
...
```bash
hermes mcp add loofah --command loofah --args mcp
...
```bash
loofah --vault-path /path/to/vault mcp
```

Apply the same argument order in a client command. In Claude Code, that becomes:

```bash
claude mcp add --transport stdio loofah -- loofah --vault-path /path/to/vault mcp
```
...
```json
{
  "command": "/absolute/path/to/loofah",
