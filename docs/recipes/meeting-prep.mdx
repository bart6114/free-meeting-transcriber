...
description: "Build a focused meeting brief from recent Free Meeting Transcriber context."
---
...
<Steps>
  <Step title="Find the latest meeting">
    ...
    ```bash
    fmtr --json meetings list --query "project sync" --limit 10
    ```
  ...
  <Step title="Review recurring history">
    ...
    ```bash
    fmtr --json meetings history MEETING_ID --limit 5 --offset 0
    ```
