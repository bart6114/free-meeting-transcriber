...
Creates a meeting from an audio file and prints its id. The audio is converted to the vault's 16 kHz MP3 format, exactly as the desktop app's import does, so a running app picks the meeting up automatically. Accepts `wav`, `mp3`, `ogg`, `mp4`, `m4a`, `flac`, `webm`, and `aac` files; `--title` defaults to the audio file's name. Transcription does not run — open the meeting in the app to transcribe it.
