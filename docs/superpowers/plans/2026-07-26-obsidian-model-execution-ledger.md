...
| F — search on files | in progress | — | E3 findings: empty-note placeholder was actively harmful (shadowed real note in search) — deleted; tag tables have zero readers — SQL tag writes dead; supersede = session_replace_transcripts (hard-deletes SQL rows, trashes prior file). FE has ZERO SQL; plugins/db reachable only via unimported src/db/index.ts. |
