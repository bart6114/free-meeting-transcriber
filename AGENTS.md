...
- Caveats: concurrent builds from different worktrees serialize on cargo's file lock, the cache grows as branches accumulate artifacts (`rm -rf ~/.cache/fmtr` resets it at the cost of one cold build), and deleting a worktree no longer frees build space.
...
- Always ship stable releases through the `desktop-release` skill rather than dispatching the workflow directly — it writes the `packages/changelog/content/<version>.md` release notes (which the in-app "What's new" dialog needs) and fills the GitHub release body before building.
