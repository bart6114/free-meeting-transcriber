        ...
        .param_changed(|_, user_data, id, param| {
            ...
            if user_data.format.parse(param).is_ok() {
                ...
                if rate > 0 {
                    ...
                    tracing::info!(
                        fmtr.audio.sample_rate_hz = rate,
...
fn pulseaudio_capture_loop(
    ...
    let mut context = PaContext::new(&mainloop, "fmtr-speaker-capture")
    ...
    let setup_result = (|| -> Result<_> {
        ...
        tracing::info!(fmtr.audio.device = %monitor_device, "connecting_to_monitor_device");

        mainloop.lock();
        let stream_result = (|| -> Result<_> {
            let mut stream = PaStream::new(&mut context, "fmtr-capture", &spec, None)
    ...
    tracing::info!(
        fmtr.audio.sample_rate_hz = actual_rate,
