...
pub(crate) fn configure_sentry_session_context(params: &SessionParams) {
    sentry::configure_scope(|scope| {
        scope.set_tag("fmtr.session.id", &params.session_id);
        scope.set_tag(
            "fmtr.session.type",
        ...
        session_context.insert(
            "fmtr.session.id".to_string(),
            params.session_id.clone().into(),
        );
        session_context.insert(
            "fmtr.gen_ai.request.model".to_string(),
            params.model.clone().into(),
        );
        session_context.insert(
            "fmtr.session.transcription_mode".to_string(),
            format!("{:?}", params.transcription_mode).into(),
        );
        session_context.insert(
            "fmtr.session.onboarding".to_string(),
            params.onboarding.into(),
        );
        session_context.insert(
            "fmtr.session.language_codes".to_string(),
...
pub(crate) fn clear_sentry_session_context() {
    sentry::configure_scope(|scope| {
        scope.remove_tag("fmtr.session.id");
        scope.remove_tag("fmtr.session.type");
        scope.remove_context("fmtr.session");
    });
}
...
pub(crate) fn emit_session_ended(
    ...
    if let Some(reason) = failure_reason {
        tracing::info!(fmtr.session.stop_reason = %reason, "session_stopped");
    } else {
