...
impl RealtimeSttAdapter for FmtrAdapter {
    ...
    fn is_supported_languages(
        ...
        FmtrAdapter::is_supported_languages_live(languages, model)
    }
...
mod tests {
    ...
    const API_BASE: &str = "http://localhost:8787/stt";
        ...
        run_url_test_cases(
            ...
            &[
                UrlTestCase {
                    ...
                    model: Some("nova-3"),
                    languages: &[ISO639::En],
                    contains: &["localhost", "listen", "model=nova-3", "language=en"],
                    not_contains: &[],
                },
                UrlTestCase {
                    name: "multi_language",
    ...
    fn test_url_with_keywords() {
        let adapter = FmtrAdapter::default();
        let params = owhisper_interface::ListenParams {
            model: Some("nova-3".to_string()),
            languages: vec![ISO639::En.into()],
            keywords: vec!["Waveform".to_string(), "transcription".to_string()],
            ..Default::default()
        };
        ...
        assert!(url_str.contains("keyword=Waveform"));
    ...
    fn test_localhost_uses_ws_scheme() {
        ...
        let url = adapter.build_ws_url("https://example.com/stt", &params, 1);
    ...
    fn test_provider_param_preserved_in_url() {
        let adapter = FmtrAdapter::default();
        let base_with_provider = "http://localhost:8787/stt?provider=fmtr";
        let params = owhisper_interface::ListenParams {
            model: Some("cloud".to_string()),
            languages: vec![ISO639::En.into()],
            ..Default::default()
        ...
        assert!(
            url.as_str().contains("provider=fmtr"),
            "provider=fmtr query param should be preserved in the final URL"
        );
    }
