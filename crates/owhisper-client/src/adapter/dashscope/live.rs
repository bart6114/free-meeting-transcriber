...
impl RealtimeSttAdapter for DashScopeAdapter {
    ...
    fn initial_message(
        ...
        tracing::debug!(
            fmtr.payload.size_bytes = json.len() as u64,
    ...
    fn parse_response(&self, raw: &str) -> Vec<StreamResponse> {
        ...
        match event {
            DashScopeEvent::SessionCreated { session } => {
                tracing::debug!(
                    fmtr.stt.provider_session.id = %session.id,
            ...
            DashScopeEvent::InputAudioBufferSpeechStarted { item_id } => {
                tracing::debug!(fmtr.stt.item.id = %item_id, "dashscope_speech_started");
                vec![]
            }
            DashScopeEvent::InputAudioBufferSpeechStopped { item_id } => {
                tracing::debug!(fmtr.stt.item.id = %item_id, "dashscope_speech_stopped");
                vec![]
            }
            DashScopeEvent::ConversationItemInputAudioTranscriptionCompleted {
                ...
                tracing::debug!(
                    fmtr.stt.item.id = %item_id,
                    fmtr.transcript.char_count = transcript.chars().count() as u64,
            ...
            DashScopeEvent::ConversationItemInputAudioTranscriptionText {
                ...
                tracing::debug!(
                    fmtr.stt.item.id = %item_id,
                    fmtr.transcript.char_count = text.chars().count() as u64,
