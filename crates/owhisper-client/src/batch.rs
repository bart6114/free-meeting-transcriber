...
use crate::adapter::{BatchSttAdapter, append_provider_param, is_fmtr_proxy};
...
mod tests {
    use super::*;
    use crate::{DeepgramAdapter, FmtrAdapter, OpenAIAdapter};
    use hypr_language::{ISO639, Language};

    #[test]
    fn injects_provider_for_fmtr_proxy() {
        let client = BatchClient::<FmtrAdapter>::builder()
            .api_base("http://localhost:8787/stt")
            .api_key("test")
            .build();

        assert!(client.api_base.contains("provider=fmtr"));
    }
    ...
    #[test]
    fn injects_provider_for_direct_provider_adapter_on_fmtr_proxy() {
    ...
    fn rewrites_existing_provider_for_direct_provider_adapter() {
        let client = BatchClient::<OpenAIAdapter>::builder()
            .api_base("http://localhost:8787/stt?provider=fmtr&model=whisper-1")
            .api_key("test")
            .build();
